...
export const GET: RequestHandler = async ({ url, request, platform }) => {
  ...
  const clientSecret = (platform as App.Platform).env.GITHUB_CLIENT_SECRET
  const allowedIds = (platform as App.Platform).env.ALLOWED_GITHUB_IDS
  ...
  if (state == null || state === '' || (stateMatch == null) || state !== stateMatch[1]) {
  ...
  const githubUser: { id: number, login: string, name: string | null } = await userRes.json()

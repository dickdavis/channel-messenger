...
<div class="message {role}">
	<div class="bubble">
		...
		<time>{(() => { const d = new Date(createdAt.endsWith('Z') ? createdAt : createdAt + 'Z'); const pad = (n: number) => String(n).padStart(2, '0'); return `${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())} ${pad(d.getHours())}:${pad(d.getMinutes())}` })()}</time>
	</div>
</div>
